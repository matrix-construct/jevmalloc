// `build.rs` pulls this file in with `include!`, so it is not only a module.
// Nothing here may be an inner attribute: a `//!` doc or a `#![...]` lands in
// the middle of the build script and fails to parse. The module-level doc for
// these items therefore sits on the `mod env;` declaration in `lib.rs`.

/// `jemalloc` is known not to work on these targets:
pub static UNSUPPORTED_TARGETS: &[&str] =
	&["rumprun", "bitrig", "emscripten", "fuchsia", "redox", "wasm32"];
...
///
/// “it was found that the `realpath` function in libc would allocate with libc
