...
//! and retrieve statistics. This crate provides a typed API over that
//! interface.
//!
//! While `mallctl` takes a string to specify an operation (e.g.
//! `stats.allocated` or `stats.arenas.15.muzzy_decay_ms`), the overhead of
//! repeatedly parsing those strings is not ideal. Fortunately, `jemalloc`
//! offers the ability to translate the string ahead of time into a "Management
//! Information Base" (MIB) to speed up future lookups.
//!
...
use crate::std::{fmt, ops, ptr, slice, str};
...
impl epoch {
	...
	pub fn advance() -> Result<u64> { Self::update(1) }
}

impl epoch_mib {
	/// Advances the epoch returning its old value - see [`epoch`].
	pub fn advance(self) -> Result<u64> { self.0.update(1) }
}
