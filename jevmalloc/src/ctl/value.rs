...
use super::{Error, Key, Result, raw};
...
/// Reads a control backed by an immutable process-lifetime `const char *`.
///
/// A library supplied through `JEMALLOC_OVERRIDE` must preserve the bundled
/// jemalloc implementation's storage contract for these controls.
pub(crate) fn get_cstr(key: &Key) -> Result<&'static CStr> {
	// SAFETY: callers select controls that return immutable process-lifetime C
	// strings.
	let value = unsafe { raw::get::<*const c_char>(key) }?;
	if value.is_null() {
		return Err(Error::bad_address());
	}

	// SAFETY: callers select controls whose non-null pointers remain immutable,
	// readable, and NUL-terminated for the life of the process.
