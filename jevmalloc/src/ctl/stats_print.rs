...
pub struct Options {
	...
	/// If set, information about individual size classes for bins will be
	/// skipped.
	///
	/// This corresponds to the `b` character.
	pub skip_bin_size_classes: bool,

	/// If set, information about individual size classes for large objects will
	/// be skipped.
	///
...
///
/// The information is the same that can be retrieved by the individual lookup
/// methods in this crate, but all done at once.
...
{
	unsafe {
		let mut state = State { writer, error: Ok(()), panic: Ok(()) };
		let mut opts = [0; 8];
		let mut i = 0;
