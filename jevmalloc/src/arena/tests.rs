...
fn validates_index_range() {
	// SAFETY: these handles are used only for numeric validation and never
	// issue a control operation, so no arena liveness assumption is required.
	...
	// SAFETY: range validation rejects the index before a handle is
	// constructed.
